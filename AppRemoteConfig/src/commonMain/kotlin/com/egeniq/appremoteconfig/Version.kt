...
data class Version(val canonical: Triple<Int, Int, Int>) : Comparable<Version> {
    override fun compareTo(other: Version): Int = when (canonical.first) {
        other.canonical.first if canonical.second == other.canonical.second -> canonical.third.compareTo(
            other.canonical.third
        )
        other.canonical.first -> canonical.second.compareTo(other.canonical.second)
